...
:fn-tis-95: footnote:tis-95[TIS Committee.  Tool Interface Standard (TIS) Executable and Linking Format (ELF) Specification: Version 1.2.  May 1995.  http://refspecs.linuxbase.org/elf/elf.pdf.]
  ...
  ** uses a patched version of SeaBIOS,{fn-oconnor-10}{fn-seabios-21} which I call SeaBIOSify — builds as a collection of code libraries
  *** uses link-time optimization (LTO) (upstream SeaBIOS uses whole-program optimization)
  *** **note:** `biefircate` 's memory model is different from stock SeaBIOS's
  **** 32-bit code goes into extended memory (stock SeaBIOS: all code is in base memory)
  **** `VARFSEG`, `VARLOW` memory locations are fixed only at boot time (SeaBIOS: fixed at compile time)
  **** accesses from 32-bit code to `VARFSEG`, `VARLOW` vars. must go through macros (SeaBIOS: no need)
  ** calling convention for stage 2 is `-mregparm=3 -mrtd -mpreferred-stack-boundary=2`
  *** when calling non-variadic function: first few arguments go in `eax`, `edx`, `ecx`; callee pops any stack arguments
  *** `esp` is assumed to be 2² = 4 byte aligned
  *** callee must preserve `ebx` (!), `esi`, `edi`, `ebp`
  *** SeaBIOSify uses the same calling convention
